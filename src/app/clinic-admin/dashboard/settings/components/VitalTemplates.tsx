// src/app/clinic-admin/dashboard/settings/components/VitalTemplates.tsx
'use client';
...
import { Plus, Trash2, Layers, Check, ChevronDown, ChevronUp, Edit2, X } from 'lucide-react';
...
export const VitalTemplates = ({ clinicId }: { clinicId: number }) => {
  ...
  // Form State
  const [isFormOpen, setIsFormOpen] = useState(false); // Renamed from isCreating
  const [editingId, setEditingId] = useState<number | null>(null); // Track if editing
  const [templateName, setTemplateName] = useState(''); // Renamed from newTemplateName
  ...
  const openEditForm = (e: React.MouseEvent, template: Template) => {
      ...
      // Map members to the format used by the selector
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      <div className="flex justify-between items-center">
        <div>
           <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
             <Layers className="h-5 w-5 text-primary" /> Vital Templates
           </h3>
           <p className="text-sm text-gray-500">Create groups of vitals (e.g., "Cardiology Check") for quick assignment.</p>
        </div>
        {!isFormOpen && (
      ...
      {isFormOpen && (
        <Card padding="lg" className="border-2 border-blue-100 mb-6 relative">
           ...
           <div className="space-y-4">
              ...
              id="template_name"
                ...
                placeholder="e.g. Standard Physical"
              />
              
              <div className="bg-gray-50 p-4 rounded-lg">
                <label className="text-sm font-semibold text-gray-700 mb-2 block">Select Vitals for this Template:</label>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {availableVitals.map(v => {
                    ...
                    return (
                      <div key={v.id} className={`p-2 rounded border flex items-center justify-between ${isSelected ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
                        ...
                        {isSelected && (
                          <button 
                            ...
                            className={`text-xs px-2 py-0.5 rounded border ${isReq ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-gray-100 text-gray-500'}`}
                          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
         {templates.map(t => {
           ...
           // Hide the card being edited to avoid confusion
           if (editingId === t.id) return null;

           return (
            <Card 
                key={t.id} 
                className={`group cursor-pointer transition-all duration-300 ${isExpanded ? 'row-span-2 border-primary ring-1 ring-primary' : 'hover:border-primary'}`}
            ...
            >
                ...
                {isExpanded && (
                    <div className="mt-4 border-t pt-3 animate-in slide-in-from-top-2">
                        ...
                        <div className="space-y-1">
                            {t.members.map((m, idx) => (
                                <div key={idx} className="flex justify-between items-center text-sm p-1.5 hover:bg-gray-50 rounded">
                                    ...
                                    <span className={`text-xs px-1.5 py-0.5 rounded ${m.is_required ? 'bg-orange-50 text-orange-600' : 'bg-gray-100 text-gray-500'}`}>
