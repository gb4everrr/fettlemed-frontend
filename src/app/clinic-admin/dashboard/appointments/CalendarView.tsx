...
/**
 * HELPER: Converts a UTC date into a "Visual Date" for the target timezone.
 * * Why this is needed:
 * If an appointment is at 09:30 UTC (3:00 PM IST), and the clinic is in IST:
 * - Browser in US (EST): new Date() sees 04:30 AM. getHours() = 4. WRONG.
 * - This function creates a Date object where .getHours() returns 15 (3 PM)
 * regardless of the browser's location.
 */
const getZonedDate = (dateString: string | Date, timezone: string): Date => {
  const date = new Date(dateString);
  // Format the date specifically to the CLINIC's timezone
  const zonedString = date.toLocaleString('en-US', { timeZone: timezone });
  // Parse it back as a "local" date. This "shifts" the time to visual correctness.
  return new Date(zonedString);
};

/**
 * HELPER: Formats time for display (e.g., "3:00 PM") using the Clinic's Timezone
 */
...
export default function CalendarView({
  ...
  // 1. Filter appointments for a specific day using ZONED dates
  const getAppointmentsForDay = (day: Date) => {
    return appointments.filter(apt => {
      // Convert the appointment start time to Clinic Time
      const aptZoned = getZonedDate(apt.datetime_start, clinicTimezone);
      
      // Compare with the column day (Visual comparison)
      // Note: 'day' from weekDays is usually already local/midnight. 
      // We compare the date strings to be safe.
                    ...
                    return (
                      ...
                      >
                        <div style={{ background: color.bg, borderLeft: `4px solid ${color.border}`, color: color.text }} className="h-full rounded-md p-1.5 overflow-hidden text-[10px] leading-tight flex flex-col">
                          ...
                          {/* Only show doctor name if height allows */}
                          <div className="truncate hidden sm:block opacity-80">
