// src/components/clinic/modals/EditAppointmentModal.tsx
'use client';
...
import { GenerateInvoiceView } from '@/components/clinic/modals/GenerateInvoiceModal'; 
...
export function EditAppointmentModal({
  ...
  // When the modal opens or the appointment prop changes, reset to details
  useEffect(() => {
    setView('details');
    setError(null);
    setSuccessMessage(null);
    setAppointment(initialAppointment); // Ensure local state is updated
  ...
  return (
    <div 
      className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center p-4 z-50 overflow-y-auto" 
      onClick={onClose}
    >
        ...
        {view === 'details' ? (
          ...
          <div className="p-6 md:p-8 max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center text-sm text-gray-500 mt-3 mb-2"><Stethoscope className="h-4 w-4 mr-2" /><span className="font-medium">Doctor</span></div>
                  ...
                  {appointment.notes && appointment.notes.includes('[Rescheduled from:') && (
                    <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                      <div className="flex items-start"><RefreshCw className="h-5 w-5 text-amber-600 mr-2 mt-0.5" /><div><p className="text-sm font-semibold text-amber-800">Rescheduled Appointment</p><p className="text-sm text-amber-700 mt-1">{appointment.notes.match(/\[Rescheduled from: (.*?)\]/)?.[1] || 'Previously rescheduled'}</p></div></div>
                    </div>
                  )}
                </div>
              </Card>
              ...
              <Card className="shadow-lg h-full">
                <div className="p-6">
                  <div className="flex items-center mb-4"><Activity className="h-6 w-6 text-blue-600 mr-2" /><h2 className="text-xl font-bold text-gray-800">Patient Vitals</h2></div>
                  {isLoadingVitals ? (
                    <div className="text-center py-8"><Clock className="h-8 w-8 animate-spin mx-auto text-gray-400 mb-2" /><p className="text-gray-500 text-sm">Loading vitals...</p></div>
                  ) : doctorVitals.length === 0 ? (
                    <div className="text-center py-8 bg-gray-50 rounded-lg"><AlertCircle className="h-12 w-12 mx-auto text-gray-400 mb-2" /><p className="text-gray-500 text-sm">No vitals configured for this doctor</p></div>
                  ) : (
                    <div className="space-y-3">
                      <div className="overflow-hidden border border-gray-200 rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200"><tbody className="divide-y divide-gray-200 bg-white">
                          {doctorVitals.map((vital) => (
                            <tr key={vital.id}>
                              <td className="px-4 py-3 text-sm font-medium text-gray-700 whitespace-nowrap">
                                {vital.vitalConfig.vital_name}
                                {vital.is_required && <span className="text-red-500 ml-1">*</span>}
                                {vital.vitalConfig.unit && (<span className="text-gray-500 text-xs ml-1">({vital.vitalConfig.unit})</span>)}
                              </td>
                              <td className="px-4 py-3 text-sm text-gray-800 whitespace-nowrap">
                                {editingVitalName === vital.vitalConfig.vital_name ? (
                                  <Input id={`vital-${vital.id}`} type={vital.vitalConfig.data_type === 'number' ? 'number' : 'text'} value={vitalsValues[vital.vitalConfig.vital_name] || ''} onChange={(e) => handleVitalChange(vital.vitalConfig.vital_name, e.target.value)} onBlur={() => setEditingVitalName(null)} placeholder={`Enter ${vital.vitalConfig.vital_name.toLowerCase()}`} className="w-full" autoFocus />
                                ) : (
                                  <div onClick={() => { if (appointment.status !== 2) { setEditingVitalName(vital.vitalConfig.vital_name); } }} className={`min-h-[38px] p-2 -m-2 rounded-md flex items-center ${(appointment.status !== 2) ? 'cursor-pointer hover:bg-gray-100' : 'cursor-not-allowed'}`}>
                                    {vitalsValues[vital.vitalConfig.vital_name] || (<span className="text-gray-400">Click to add...</span>)}
                                  </div>
                                )}
                              </td>
                            </tr>
                      ...
                      {(appointment.status !== 2) && (
                        <Button variant="primary" onClick={handleSaveVitals} disabled={isSavingVitals} className="w-full flex items-center justify-center mt-4">
                          <Save className="h-4 w-4 mr-2" />{isSavingVitals ? 'Saving...' : vitalsEntry ? 'Update Vitals' : 'Save Vitals'}
                        </Button>
                      )}
                      {vitalsEntry && (
                        <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                          <p className="text-xs text-green-700"><strong>Last recorded:</strong> {vitalsEntry.entry_date} at {vitalsEntry.entry_time}</p>
                        </div>
                      )}
                    </div>
                  )}
              ...
              <Card className="shadow-lg h-full">
                <div className="p-6">
                  {appointment.status === 2 ? (
                    <div className="text-center"><p className="text-gray-600 mb-4">This appointment is cancelled and cannot be modified.</p><Button variant="secondary" onClick={onClose}>Close</Button></div>
                  ) : isRescheduling ? (
                    <>
                      <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold text-gray-800">Reschedule Appointment</h2><Button variant="ghost" size="sm" onClick={() => setIsRescheduling(false)}>Cancel</Button></div>
                      <div className="space-y-4">
                        <div><label className="block text-sm font-medium text-gray-700 mb-2">Select New Date</label><Input id="reschedule-date" type="date" value={rescheduleDate} onChange={(e) => setRescheduleDate(e.target.value)} min={new Date().toISOString().split('T')[0]} /></div>
                        {isLoadingSlots ? (
                          <div className="text-center py-6"><Clock className="h-8 w-8 animate-spin mx-auto text-gray-400 mb-2" /><p className="text-gray-500">Loading available slots...</p></div>
                        ) : availableSlots.length === 0 ? (
                          <div className="text-center py-6 bg-gray-50 rounded-lg"><AlertCircle className="h-12 w-12 mx-auto text-gray-400 mb-2" /><p className="text-gray-500">No available slots for this date</p></div>
                        ) : (
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">Select Time Slot</label>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-44 overflow-y-auto">
                              {availableSlots.map((slot) => (
                                <button key={slot.id} onClick={() => setSelectedSlot(slot)} className={`p-2 rounded-lg border-2 transition-all text-sm ${selectedSlot?.id === slot.id ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'}`} type="button">
                        ...
                        {selectedSlot && (<div className="bg-blue-50 p-3 rounded-lg border border-blue-200"><p className="text-sm text-blue-800"><strong>New appointment time:</strong> {rescheduleDate} at {selectedSlot.start_time} - {selectedSlot.end_time}</p></div>)}
                        <div className="flex gap-3"><Button variant="primary" onClick={handleReschedule} disabled={!selectedSlot || isSaving} className="flex-1"><RefreshCw className="h-4 w-4 mr-2" />{isSaving ? 'Rescheduling...' : 'Confirm Reschedule'}</Button><Button variant="ghost" onClick={() => { setIsRescheduling(false); setSelectedSlot(null); }}>Cancel</Button></div>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div className="flex flex-col sm:flex-row flex-wrap gap-4 items-center justify-center">
                        ...
                        {appointment.status !== 3 && (<Button variant="secondary" onClick={() => setIsRescheduling(true)} className="flex items-center justify-center"><RefreshCw className="h-4 w-4 mr-2" />Reschedule</Button>)}
                        {appointment.status !== 3 && (<Button variant="primary" onClick={handleMarkComplete} disabled={isSaving} className="flex items-center justify-center bg-green-600 hover:bg-green-700"><CheckCircle className="h-4 w-4 mr-2" />{isSaving ? 'Marking...' : 'Mark as Complete'}</Button>)}
                        {appointment.status !== 3 && (<Button variant="outline" onClick={() => setShowCancelConfirm(true)} className="flex items-center justify-center text-red-600 border-red-300 hover:bg-red-50"><XCircle className="h-4 w-4 mr-2" />Cancel Appointment</Button>)}
                        {appointment.status === 3 && (<p className="text-sm flex items-center justify-center text-gray-500 p-2">Appointment is completed.</p>)}
                      </div>
                    </>
                  )}
                </div>
              ...
              <Card className="shadow-lg h-full">
                <div className="p-6">
                  ...
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={6} className="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2" placeholder="Add any notes about this appointment..." disabled={appointment.status === 2} />
                  {(appointment.status !== 2) && (
                    <div className="flex gap-3 mt-4">
                      <Button variant="primary" onClick={handleUpdateNotes} disabled={isSaving}><Save className="h-4 w-4 mr-2" />{isSaving ? 'Saving...' : 'Save Notes'}</Button>
                      <Button variant="ghost" onClick={() => { setNotes(appointment.notes || ''); }}>Reset</Button>
                    </div>
                  )}
                </div>
              </Card>
          ...
          <GenerateInvoiceView
            ...
            onSetView={setView} // Pass the function to allow it to go "back"
          />
        )}
      </div>
    </div>
